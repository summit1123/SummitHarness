...
""",
    ...
    def test_ralph_plain_text_start_skill_aliases_summit_start(self) -> None:
        ...
        self.assertIn("역할은 실행이 아니라 온보딩", skill_text)
        self.assertIn("즉시 loop를 돌리는 명령이 아닙니다", skill_text)
    ...
    def test_ralph_start_command_requires_confirmation_before_init(self) -> None:
        ...
        self.assertIn("목적은 루프 실행이 아니라 **강한 온보딩**", text)
        self.assertIn("기본값으로 그 자리에 bootstrap하지 않습니다", text)
        self.assertIn("workflow profile을 고르기 전에 반드시 평문으로 묻습니다", text)
        self.assertIn("충분히 깊은 구조화 Q&A", text)
        self.assertIn("추측으로 선택하지 않습니다", text)
        self.assertIn("approval 문서를 실제 승인된 것처럼 쓰지 않습니다", text)
        self.assertIn("온보딩 깊이가 속도보다 중요합니다", text)
    ...
    def test_summit_start_skill_requires_waiting_when_answers_are_missing(self) -> None:
        ...
        self.assertIn("어떤 workflow profile도 결정하기 전에", skill_text)
        self.assertIn("답변이 아직 명확하지 않으면 그 자리에서 멈추고 사용자 답변을 기다립니다", skill_text)
        self.assertIn("요청이 흐릿하면 여러 번에 나누어 묻습니다", skill_text)
        self.assertIn("repo에 코드베이스가 있다는 이유만으로 `build-direct`를 강제하지 않습니다", skill_text)
        self.assertIn("초안 approval 파일을 실제 승인으로 취급하지 않습니다", skill_text)
        self.assertIn("속도를 위해 온보딩을 압축하지 않습니다", skill_text)
        self.assertNotIn("Ask only the minimum onboarding questions needed to lock:", skill_text)

    def test_summit_start_init_places_user_intent_first_in_onboarding(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp).resolve()
            subprocess.run([sys.executable, str(BOOTSTRAP), str(root)], check=True)
            ...
            onboarding = (root / ".codex-loop" / "workflow" / "ONBOARDING.md").read_text(encoding="utf-8")
            self.assertIn("### C1. 이번 런에서 지금 사용자가 하고 싶은 일은 무엇입니까?", onboarding)
