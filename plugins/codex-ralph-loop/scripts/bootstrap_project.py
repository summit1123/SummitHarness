...
def main() -> int:
    ...
    print('  2. Codex 안에서는 `ralph start` 또는 `/ralph start`로 온보딩 인터뷰를 시작합니다.')
    print('  3. 먼저 사용자에게 이번 런에서 무엇을 하고 싶은지와 어디서 멈추고 싶은지 확인합니다.')
    print('  4. profile, goal, scope, approval path, evidence bar가 명확해질 때까지 task graph를 만들지 않습니다.')
    print('  5. 온보딩 답변이 명확해진 뒤에만 `python3 scripts/summit_start.py init --profile <...> --goal "<...>"` 를 사용해 workflow 문서를 생성합니다.')
    print('  6. `.codex-loop/workflow/ONBOARDING.md`를 채우고, 필요하면 `.codex-loop/workflow/IDEAS.md`도 정리합니다.')
    print('  7. `python3 scripts/summit_intake.py init --mode <proposal|prd|implementation|product-ui>` 를 실행합니다.')
    print('  8. 요청자 Q&A를 `.codex-loop/intake/ANSWERS.md`에 기록하고 `.codex-loop/intake/APPROVAL.md`에서 승인을 잠급니다.')
    print('  9. `python3 scripts/summit_research.py init --mode <proposal|prd|implementation|product-ui>` 를 실행합니다.')
    print(' 10. 단계형 리서치 계획을 작성하고 `.codex-loop/research/APPROVAL.md`에서 승인을 잠급니다.')
    print(' 11. 승인된 리서치 방향에 맞춰 `.codex-loop/prd/PRD.md`, `SUMMARY.md`, `.codex-loop/design/DESIGN.md`를 확정합니다.')
    print(' 12. `.codex-loop/design/reference-packs/`에서 레퍼런스 팩을 고르거나 커스터마이징하고 `DESIGN.md`에 기록합니다.')
    print(' 13. 제안서/제출 흐름이라면 먼저 `docs/submissions/proposal.md`를 편집합니다.')
    print(' 14. `python3 scripts/review_submission_source.py docs/submissions/proposal.md`로 원고 리뷰를 돌립니다.')
    print(' 15. 원고 게이트를 통과하면 `python3 scripts/render_markdown_submission.py`로 렌더링합니다.')
    print(' 16. `python3 scripts/ralph_stage_gate.py init`으로 all-rounder stage gate spec/results 디렉터리를 준비합니다.')
    ...
    print(' 17. `.codex-loop/config.json`에 실제 local build, lint, test, screenshot 명령을 추가합니다.')
