...
4. Ask a short onboarding Q&A that decides:
...
5. If the answers are not already explicit in the prompt or attached materials, stop after asking and wait for the user response. Do **not** choose `build-direct`, `idea-to-service`, or any other profile by guess.
6. Do not run `python3 scripts/summit_start.py init ...` in the same turn unless the workflow profile, goal sentence, scope, and evidence bar are already explicit or the user has just confirmed them.
...
11. Do not start the autonomous Ralph loop until the user has enough clarity to approve the first stage honestly.
