# Mode Contract: Proposal

Use this mode for proposal packages, contest submissions, reviewer-facing documents, and business narratives.

## Source of truth

- `docs/submissions/proposal.md`
- `.codex-loop/design/DESIGN.md`
- `.codex-loop/prd/PRD.md`

## Completion bar

- The Markdown source reads like a reviewer-facing document, not a chat response.
- Problem, solution, feasibility, business path, and expected effect are all explicit.
- Needed tables, evidence captures, comparisons, and workflows exist in source form.
- The rendered PDF is a packaging step, not the only place where quality appears.

## Fail when

- The source is thin and the layout is doing all the work.
- The document sounds like an assistant explaining itself.
- Pages are sparse or box-driven without carrying information.
- Current implementation proof and future expansion are blurred together.
